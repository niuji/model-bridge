...
pub async fn fetch_balance(
    ...
    match usage.adapter.as_str() {
        ...
        "bigmodel" => bigmodel_usage(client, api_key, &usage.params).await,
...
mod tests {
    ...
    async fn bigmodel_wallet_maps_camel_to_snake_with_bearer() {
        ...
        Mock::given(method("GET"))
            .and(path("/wallet"))
            .and(header("Authorization", "Bearer id.secret"))
            .respond_with(ResponseTemplate::new(200).set_body_json(json!({
    ...
    async fn bigmodel_rejects_params_without_network() {
        ...
        let err = fetch_balance(&client(), &usage_def("bigmodel", &p), "k").await.unwrap_err();
        assert!(err.to_string().contains("unknown usage param"));
    }
}
