...
async function loadProviders() { loading.value = true; try { const res = await fetch(`${API_BASE}/providers`); const list: ProviderSummary[] = await res.json(); providers.value = list.sort((a, b) => a.name.localeCompare(b.name)) } finally { loading.value = false } }
