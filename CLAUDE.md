...
Provider definitions (id, name, icon, channels) are embedded at compile time via `include_str!("../providers.json")` in `config.rs::load_providers()` — there is no runtime disk read of this file. User-level overrides (`~/.mb/providers.json`, same schema) are loaded and merged at startup: a same-`id` entry is **shallow-merged over** the builtin one (user-written keys win; omitted keys — e.g. `channels` — are inherited from the builtin; an explicit `null` removes), new `id` is appended. User-specific credentials and per-model toggles (API keys, enabled state, model whitelists) are stored in SQLite. At runtime, the three layers are merged to build the in-memory route table.
...
2. **`~/.mb/providers.json`** (user-local overrides) — same schema as `providers.json`. Loaded and merged into layer 1 at startup: same `id` shallow-merges over the builtin entry (user keys win, omitted keys inherit — so an override can add just a `usage` block without repeating `channels`; explicit `null` removes a key), new `id` is appended. File-level failures (unreadable / bad JSON / an entry missing its string `id` / a merged entry that no longer deserializes) are logged and the whole file skipped; the file is optional. Use this for private or corporate providers that shouldn't be committed to the repo.
...
Before forwarding, the proxy makes these modifications to the client's request — this is the **complete, closed list**; adding any further body modification violates the pass-through principle (see [Key Design Decisions](#key-design-decisions)):
...
4. **Header passthrough**: An **exclusion list, not a whitelist** — every client header is forwarded except those in `request_log::NON_FORWARDED_HEADERS`. Never revert this to a whitelist: Claude Code adds capability headers and paired body fields every release, and its auto-mode server-side checks depend on `safeguards`/`safeguard_results` passing through untouched — a pinned list strips the next capability silently. The exclusions are load-bearing: `accept-encoding` (reqwest is built with `default-features = false` and no gzip/brotli feature, so a compressed upstream response would silently break SSE line parsing and usage extraction while still passing through to the client), `authorization`/`x-api-key` and `anthropic-workspace-id` (reqwest's `.header()` appends rather than replaces, so forwarding would put the client value alongside the gateway-injected one as a second header of the same name), `content-length` (the body is rewritten, so the client's length is stale), plus `cookie`, `host`, and the hop-by-hop headers.
...
- **Pass-through principle（只转发，不修改，强制规则）.** The three proxy endpoints (`/openai-chat`, `/openai-responses`, `/anthropic`) forward LLM request and response bodies **as-is**. Never add new request/response transformations — no role rewrites (e.g. `developer`→`system`), no parameter injection/removal beyond the existing list, no response body rewriting. The modifications documented in [Proxy Request Modifications](#proxy-request-modifications) are the complete, closed set of allowed request changes; client↔upstream protocol mismatches (e.g. an upstream rejecting the `developer` role) are the caller's to resolve — the gateway routes, it is not a protocol-compatibility layer.
...
- **Provider balance probing.** `ProviderDef.usage`（providers.json / ~/.mb/providers.json）把 provider 绑定到内置余额 adapter（`balance_svc`，按名 match 分发；鉴权/endpoint/解析差异封在各 adapter 内）。后台任务按 `bridge.balance_interval_min`（默认 10）探测启用的 provider，UPSERT `provider_balance` 单行快照；载荷为 adapter 自定义 JSON，失败保留上次成功值。前端按 adapter 名分发渲染；用户本地 provider（如 trip）用声明式 `http` adapter（`params.url`/`headers` + `result` 切片）+ `display` 模板接入，见 spec `2026-08-18-provider-balance-custom-adapter.md`。bigmodel 必须走内置 adapter：它并发查两个未公开端点合并为单快照复合载荷 `{wallet, plan}`（智谱鉴权失败返回 HTTP 200 + body `code:1000/1001/401`，只看 status 的 http adapter 会误判成功；且账号常只持有钱包或 Coding Plan 之一，任一子项成功即落 ok、失败子项置 null，双失败才落 error）。 volcengine 同为内置复合 adapter（`{balance, plan_source, plan}`：费用中心现金余额 + Agent/Coding Plan 配额双探测回落），三路全走火山 OpenAPI V4 签名，AK/SK 配在 `usage.params`——方舟推理 key 调不了 billing/控制面网关；网关凭据错误常 400+`ResponseMetadata.Error` 信封，须信封优先于 status 判断，见 spec `2026-08-29-volcengine-balance-adapter-design.md`。
...
| `src/admin/balance_svc/` | 余额 adapter 目录模块：`mod.rs`（dispatch/DB 落库/探测）+ per-adapter 文件（`deepseek/openrouter/bigmodel/http/volcengine.rs`，实现与测试同文件） |
