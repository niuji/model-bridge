...
- **Provider balance probing.** `ProviderDef.usage`（providers.json / ~/.mb/providers.json）把 provider 绑定到内置余额 adapter（`balance_svc`，按名 match 分发；鉴权/endpoint/解析差异封在各 adapter 内）。后台任务按 `bridge.balance_interval_min`（默认 10）探测启用的 provider，UPSERT `provider_balance` 单行快照；载荷为 adapter 自定义 JSON，失败保留上次成功值。前端按 adapter 名分发渲染；用户本地 provider（如 trip）用声明式 `http` adapter（`params.url`/`headers` + `result` 切片）+ `display` 模板接入，见 spec `2026-08-18-provider-balance-custom-adapter.md`。
